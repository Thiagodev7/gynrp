	...
	while true do
		...
		for k,v in pairs(dropList) do
			...
			if distance <= 150 then
				DrawMarker(25,v.x,v.y,cdz+0.01,0,0,0,0,0,0,0.3,0.3,0.4,255,255,255,15,0,0,2,0,0,0,0)
				DrawMarker(21,v.x,v.y,cdz+0.35,0,0,0,0,180.0,130.0,0.3,0.3,0.3,255,150,0,50,1,0,0,1)
				if distance <= 1.2 and v.count ~= nil and v.name ~= nil and not IsPedInAnyVehicle(ped) then
					drawTxt("PRESSIONE  ~g~E~w~  PARA PEGAR~y~ "..v.count.."X "..string.upper(v.name),4,0.5,0.93,0.50,255,255,255,180)

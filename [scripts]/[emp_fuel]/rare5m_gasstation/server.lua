        ...
        if c then
            ...
            if t then
                ...
                if query and query[1] then
                    if parseInt(query[1].user_id) == t then
    ...
    function(r)
        if c then
            ...
            if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                money = vRP.getMoney(t)
            else
                money = vRP.getMoney(t)
            end
            if money >= u then
                if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                    vRP.tryFullPayment(t,u)
                else
                    vRP.tryFullPayment(t,u)
                end
                local o =
...
AddEventHandler(
    ...
    function(r)
        ...
        if t then
            ...
            if v ~= nil and parseInt(v.user_id) == t then
...
AddEventHandler(
    ...
    function(r, z, C, D, type, x)
        ...
        if x then
            D = 0
            z = tonumber(x.amount)
            local s = vRP.getUserId(source)
            if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                vRP.giveMoney(s,tonumber(x.reward) or 0)
            else
                vRP.giveMoney(s,tonumber(x.reward) or 0)
            end
    ...
    function(r)
        ...
        if b[source] == nil then
            ...
            if t then
                local o = "SELECT user_id FROM `gas_station_business` WHERE gas_station_id = @gas_station_id"
                local query = MySQL.Sync.fetchAll(o, {["@gas_station_id"] = r})[1]
                if parseInt(query.user_id) == t then
                    local o = "DELETE FROM `gas_station_business` WHERE gas_station_id = @gas_station_id;"
                    MySQL.Sync.execute(o, {["@gas_station_id"] = r})
                    local o = "DELETE FROM `gas_station_balance` WHERE gas_station_id = @gas_station_id;"
                    MySQL.Sync.execute(o, {["@gas_station_id"] = r})
                    ...
                    MySQL.Sync.execute(o, {["@gas_station_id"] = r})
                    if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                        vRP.giveMoney(t,Config.gas_station_locations[r].sell_price)
                    else
                        vRP.giveMoney(t,Config.gas_station_locations[r].sell_price)
                    end

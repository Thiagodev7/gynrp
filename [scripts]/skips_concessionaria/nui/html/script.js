...
function menuVehicle(event) {
    ...
    $.post('http://skips_concessionaria/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}
    ...
    $('#closemenu').append(`
        ...
        <div class="modal-footer">
            <div class="modal-price">
                <p class='price sale'>R$ `+ parseFloat(vehicleSelected.sale).toFixed(3)+`</p> 
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('http://skips_concessionaria/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('http://skips_concessionaria/Close');
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('http://skips_concessionaria/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('http://skips_concessionaria/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('http://skips_concessionaria/rotate', JSON.stringify({key: _key}))
    }
            ...
            if(!itemDisabled) {
                ...
                $('#contentVehicle').append(`
                    ...
                    <div class="row spacebetween">
                        ...
                        <span>`+dataCar[4].outerText+` KM/H</span>
                    </div>

                    <div class="row spacebetween">
                        <span class="title">Desempenho</span>
                        <span>`+dataCar[5].outerText+` CV</span>
                    </div>

                    <div class="row spacebetween">
                        <span class="title">Preço</span>
                        <span>R$`+ dataCar[6].outerText+`</span>
                    </div>
                ...
                $.post("http://skips_concessionaria/SpawnVehicle", JSON.stringify({ modelcar: dataCar[8].outerText, price: dataCar[6].outerText }));
            }
        break;
    }
});
...
//         $.post("http://skips_concessionaria/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
//     } else {
//         $.post("http://skips_concessionaria/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
//     }

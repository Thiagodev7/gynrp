...
Config.setMultas = function (source, nuser_id, multa)
   -- local uMultas = vRP.query("vrp_policia/selectMultas",{ user_id = nuser_id })
 --   local uMultas = vRP.getUData(parseInt(nuser_id), 'vRP:multas')

    local consult = vRP.query("SkS/selectMultas",{ user_id = nuser_id })
    vRP.execute("SkS/multarSKS",{ user_id = nuser_id, multas = consult[1].multas+multa}) 
    --vRP.setUData(parseInt(nuser_id), 'vRP:multas', json.encode(parseInt(multas) + parseInt(multa)))
end

Config.getMultas = function (nuser_id)
    local consult = vRP.query("SkS/selectMultas",{ user_id = nuser_id })
  --  local uMultar = vRP.getUData(parseInt(nuser_id), 'vRP:multas')
    return json.decode(consult[1].multas) or 0
end

...
vRP._prepare("vRP/updateDiscord","UPDATE vrp_user_identities SET discord = @discord WHERE user_id = @user_id")
...
function playerSpawn(user_id, source, first_spawn)
    ...
    -- print("USERID: "..dump(user_id))
    local discord = getPlayerDiscord(source)
    -- print("DISCORD: "..dump(discord))

	if discord then
      	-- print("ATUALIZANDO DISCORD")
		vRP.execute("vRP/updateDiscord",{ discord = discord, user_id = user_id })
	end

    if first_spawn then
--        print ("O id "..user_id.." passou pelo Creator!")
        vRP.addUserGroup(1,"admin.permissao")
        TriggerEvent("skips:resetdimensao")
        Wait(1000)
...
RegisterServerEvent("nation_creator:updateDiscord")
AddEventHandler("nation_creator:updateDiscord", function()
    local source = source
    local user_id = vRP.getUserId(source)

    -- print("USERID2: "..dump(user_id))
    local discord = getPlayerDiscord(source)
    -- print("DISCORD2: "..dump(discord))

	if discord then
        -- print("ATUALIZANDO DISCORD2")
        vRP.execute("vRP/updateDiscord",{ discord = discord, user_id = user_id })
	end
end)
...
function processSpawnController(source,char,user_id)
	...
	if char then
        ...
        if data and data ~= "" then 
            ...
            TriggerEvent("skips:setardimensao")
        end
	end
end


RegisterServerEvent("skips:setardimensao")
AddEventHandler("skips:setardimensao",function(id)
    local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
RegisterServerEvent("skips:resetdimensao")
AddEventHandler("skips:resetdimensao",function()
	local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
function func.getCharsInfo()
    ...
    for k,v in ipairs(data) do
        ...
        info.chars[k] = {
            name = teste[1].name.." "..teste[1].firstname, age = teste[1].age.." anos", clothes = clothes,

...
cfg.deathNUItimer = 340 -- Tempo de morte que o jogador vai conseguir utilizar o botao de desistir imediatamente
...
cfg.cooldown = 0 -- Delay por pessoa, para evitar FLOODS de Logs no DISCORD [ OBS: Deixar sempre maior que 10 ]
cfg.respawn = vec3(-436.07, -359.74, 34.95) -- Localização quando o jogador respawnar
...
cfg.clearAccount = function() -- Limpar a conta do player quando morrer
    ...
    TriggerEvent("awidj21312390wdn",0) -- ADICIONAR EVENTO DE COLETE
end
...
cfg.drawtext = function(deathtimer) -- Caso você não use a versão com NUI configurar aqui
    if deathtimer > 0 then
        drawTxt("VOCE ESTÁ INCONSCIENTE, TEM ~b~"..deathtimer.." ~w~SEGUNDOS DE VIDA",4,0.5,0.93,0.50,255,255,255,255)
...
cfg.dependencys = function() -- Coloque aqui suas dependencias de scripts para não dar conflito ex: nation_paintball, arenapvp
    ...
    if inRobbery then -- by suricato (15:28 27/03)
        return
    end
...
cfg.reasons = {
    ...
    [GetHashKey('WEAPON_ASSAULTRIFLE_MK2')] = "Arma AK-47 MK2",
}
    ...
    while true do
        ...
        if ativado then
            ...
            for k,v in pairs(allCombats) do
                ...
                if distance <= 10 then
                    time = 5
                    DrawText3D(v.coords[1],v.coords[2],v.coords[3], "~r~Deslogou | "..v.time.." ~n~ ~w~ID: ".. k .."~n~ Mensagem: "..v.motivo)
                end
            end
        end
